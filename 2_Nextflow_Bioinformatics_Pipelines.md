...
Nextflow pipelines typically produce copious files.  Having said that, users normally need to confine their instpection to only a small subset of these. Firstly we recommend that users check the execution reports just check the pipeline proceeded as expected to double-check that the correct pipeline parameters were specified.

After verifiying that a pipeline proceeded as expected, users should check the QC reports, particularly the MultiQC reports.  This point is worth stressing once again: check the QC results thorougly.  It is all too common for researchers to overlook basic quality controls and instead start analysing the data; the saying garbage in garbage applies here and if you overlook this point all your findings and conclusions could be the result of an experimental artefact.

The QC examples below were generated by MultiQC, a software tool used by most nf-core pipelines.  The graphs reported give an overview of the sequencing quality and how efficiently the FASTQ reads were mapped to a pre-specified refercence genome.
